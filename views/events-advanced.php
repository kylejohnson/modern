...
xhtmlHeaders(__FILE__, 'Advanced Search' );
?>
<body>
    ...
    <div id="content">
      <form name="contentForm" id="contentForm" method="get" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <table id="fieldsTable" class="filterTable" cellspacing="0">
          <tbody>
			<?php for ( $i = 0; $i < count($_REQUEST['filter']['terms']); $i++ ) { ?>
            <tr>
				<?php if ( $i == 0 ) { ?>
					<td>&nbsp;</td>
				<?php } else { ?>
				...
				<?php } elseif ( $_REQUEST['filter']['terms'][$i]['attr'] == "DateTime" ) { ?>
				...
				<?php } elseif ( $_REQUEST['filter']['terms'][$i]['attr'] == "Date" ) { ?>
					<td><?= buildSelect( "filter[terms][$i][op]", $opTypes ); ?></td>
				...
				<?php } elseif ( $_REQUEST['filter']['terms'][$i]['attr'] == "Weekday" ) { ?>
					<td><?= buildSelect( "filter[terms][$i][op]", $opTypes ); ?></td>
					<td><?= buildSelect( "filter[terms][$i][val]", $weekdays ); ?></td>
				<?php } elseif ( false && $_REQUEST['filter']['terms'][$i]['attr'] == "MonitorName" ) {
						$monitors = array();
						foreach ( dbFetchAll( "select Id,Name from Monitors order by Sequence asc" ) as $monitor ) {
							if ( visibleMonitor( $monitor['Id'] ) ) $monitors[$monitor['Name']] = $monitor['Name'];
						}
				?>
					<td><?= buildSelect( "filter[terms][$i][op]", $opTypes ); ?></td>
...
?>
        ...
        <div id="contentButtons">
          <input type="button" value="<?= $SLANG['Submit'] ?>" onclick="submitToEvents( this );"/>
