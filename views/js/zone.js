...
function updateZoneImage()
{
    ...
    var query = new Request.JSON( { url: thisUrl, method: 'post', timeout: AJAX_TIMEOUT, data: parms, onSuccess: updateZoneImageResponse } );//    var query = new Request.JSON( { url: thisUrl, method: 'post', timeout: AJAX_TIMEOUT, data: parms, onComplete: updateZoneImageResponse } );
    query.send();
}
...
function drawZonePoints()
{
    ...
    for ( var i = 0; i < zone['Points'].length; i++ )
    {
        ...
        div.inject( $('imageFrame') );//        div.injectInside( $('imageFrame') );
    ...
    for ( var i = 0; i < zone['Points'].length; i++ )
    {
        ...
        cell.set( 'text', i+1 );
        cell.inject( row );//        cell.injectInside( row );
        ...
        input.inject( cell );//        input.injectInside( cell );
        cell.inject( row );//        cell.injectInside( row );
        ...
        cell = new Element( 'td' );
        new Element( 'a', { 'href': '#', 'events': { 'click': addPoint.pass( i ) } } ).set( 'text', '+' ).inject( cell );//        new Element( 'a', { 'href': '#', 'events': { 'click': addPoint.pass( i ) } } ).set( 'text', '+' ).injectInside( cell );
        if ( zone['Points'].length > 3 )
            new Element( 'a', { 'id': 'delete'+i, 'href': '#', 'events': { 'click': delPoint.pass( i ) } } ).set( 'text', '-' ).inject( cell );//            new Element( 'a', { 'id': 'delete'+i, 'href': '#', 'events': { 'click': delPoint.pass( i ) } } ).set( 'text', '-' ).injectInside( cell );
        cell.inject( row );//        cell.injectInside( row );

        row.inject( tables[i%tables.length].getElement( 'tbody' ) );//        row.injectInside( tables[i%tables.length].getElement( 'tbody' ) );
    }
}
